$(function () {
    $("#jqGrid").jqGrid({
        url: '../wallet/details/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', width: 60, key: true },
			{ label: '会员名', name: 'memberName', width: 80 }, 
			{ label: '登陆ID', name: 'mobile', width: 80 }, 
			{ label: '角色', name: 'roleName', width: 100 },
			{ label: '类型', name: 'typeName', width: 30},
			{ label: '变更金额(元)', name: 'amount', width: 50 }, 			
			{ label: '账户余额(元)', name: 'availableAmount', width: 50 },
	...
	methods: {
		query: function () {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: { 'memberName': vm.q.memberName,'loginName': vm.q.loginName, 'mobile': vm.q.mobile, 'createTimeStart': vm.q.createTimeStart, 'createTimeEnd': vm.q.createTimeEnd },
                page: page
            }).trigger("reloadGrid");
		},		
		del: function (event) {
