package test.edianniu.cs.tcp.until;
...
public class FileUtils {
	...
	public static void copy(File source, File target){
		...
		{
			logger.error("Error while copying file '"+source.getAbsolutePath()+"' to file '"+target.getAbsolutePath()+"'.");
		}
	}
	...
	public static void copy(File source, String destPath){
		...
		if(destPath != null)
		{
			try
			{
				...
				FileInputStream		file_input_stream = new FileInputStream(source);
				FileOutputStream	file_output_stream = new FileOutputStream(destDir.getPath() + File.separator + source.getName());
    ...
    /**
     * 把基于网站的路径转换为为基于文件系统的路径
     * 注意文件系统的分隔符 UNIX的是 '/' 而Windows的是 '\\'
     */
	 ...
	 * 
	 * @param path
	 *            String 如：E:/work/svn/my_work/TestDate-Log4j/logs/
	 * @param target
	 *            String 如：E:/work/svn/my_work/TestDate-Log4j/logs.zip
	 */
