 ...
 *
 */
public class OnOffBiz extends BaseBiz<OnOffRequest, OnOffResponse> {
	private static final Logger logger = LoggerFactory
			.getLogger(OnOffBiz.class);
	@Autowired
	...
	class RecvReq {
		...
		Class<?> accept(FiniteStateMachine fsm, FSMContext ctx,
				OnOffRequest req) {
			logger.debug("OnOffRequest recv req : {}", req);
			OnOffResponse resp = (OnOffResponse) initResponse(ctx, req, new OnOffResponse());
			...
			SocialWorkOrderOnoffReqData reqData=new SocialWorkOrderOnoffReqData();
			BeanUtils.copyProperties(req, reqData);
			Result result=socialWorkOrderInfoService.onoff(reqData);
