var vm = new Vue({
    ...
    created: function() {
        ...
        $.ajax({
            ...
            success: function(r) {
                if (r.code == 0) {
                    if(r.companyBankCard.account == null){
                        self.bankCard.banks = r.companyBankCard.banks;
                        self.bankCard.uid = r.companyBankCard.uid;
                        self.bankCard.status = r.companyBankCard.status;
                        self.bankCard.companyName = r.companyBankCard.companyName;
                        return;
                    }
                    self.bankCard = r.companyBankCard;
                    ...
                    if (r.companyBankCard.provinceName) {
                        ...
                        $.ajax({
                            url: '../basearea/citys',
                            type: 'post',
                            data: { 'provinceId': r.companyBankCard.provinceId },
                            dataType: 'json',
                            contentType: "application/x-www-form-urlencoded",
                            success: function(r) {
                                if (r.code == 0) {
