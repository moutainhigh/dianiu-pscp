var myElectrican = [
    ...
    '    <div class="col-sm-1">',
    '       <input type="text" @input="pushQuantity" class="form-control" placeholder="请输入人数" v-model="getMydata.quantity" maxlength="2" data-vv-name="人数" v-validate="\'required|myint\'" :class="{\'is-danger\': errors.has(\'人数\')}" />',
    '    </div>',
    '    <div class="col-sm-1 control-label"><span style="color: red;">*</span>费用(元/人/天):</div>',
    '    <div class="col-sm-2">',
    '        <input type="text" @input="pushFee" class="form-control" v-model="getMydata.fee" placeholder="请输入费用" maxlength="5" data-vv-name="费用" v-validate="\'required|money\'" :class="{\'is-danger\': errors.has(\'费用\')}" />',
    '    </div>',
    '</div>',
    '<div class="form-group">',
    ...
    '    </div>',
    '    <div class="col-sm-2">',
    '        <input :id="startTimeId" class="form-control form_datetime tem-picker" size="16" type="text" data-date-format="yyyy-mm-dd" readonly placeholder="工作开始日期" :class="{\'is-danger\': errors.has(\'工作开始日期\')}" >',
    '        <input type="hidden" v-model="getMydata.beginWorkTime" @change="pushStartTime" data-vv-name="工作开始日期" v-validate="\'required\'" >',
    '    </div>',
    '    <div class="col-sm-2">',
    '        <input :id="endTimeId" class="form-control form_datetime tem-picker" size="16" type="text" data-date-format="yyyy-mm-dd" readonly  placeholder="工作结束日期" :class="{\'is-danger\': errors.has(\'工作结束日期\')}" >',
    ...
    '    <div class="col-sm-2" >',
    '        <input :id="expiryTimeId" class="form-control form_datetime tem-picker-cut" size="16" type="text" data-date-format="yyyy-mm-dd" readonly placeholder="派单截止日期" :class="{\'is-danger\': errors.has(\'派单截止日期\')}">',
        ...
        onChangeDatePicker: function() {
            ...
            $('#' + self.startTimeId).on('changeDate', function(ev) {
                if (ev.date) {
                    ...
                    var z = new Date(getNowFormatDate(self.getMydata.expiryTime)).getTime();
                    if (y < x) {
                        alert('结束时间必须晚于开始时间!');
                        $(this).val('');
            ...
            if (i < length) {
                allComponent[i].$validator.validateAll().then(function() {
                    ...
                    if (i == length) {
                        ...
                        var arr = vm.recruitList;
                        var data = {
                            'orderId': vm.orderId,
                            'recruitList': vm.recruitList
                        };
                        $.ajax({
                            type: "POST",
