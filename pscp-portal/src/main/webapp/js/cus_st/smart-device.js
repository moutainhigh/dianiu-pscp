...
Vue.component('date-pick-month-fee', {
    template: datePickerMonthStr,
    data: function () {
        return {
            value: '',
    ...
    created: function() {
        var self = this;
        this.$root.$on('clear-month-fee', function() {
          self.value = ''
        })
      }
});
...
var myTabs = [
  ...
  '           <a :id="\'test\' + tab.status"><i :class="\'fa \' + tab.icon"></i>{{ tab.name }}</a>',
  '       </li>',
  '   </ul>',
    ...
    toggleTab: function(e) {
      ...
      if (e.status > -1) {
        ...
        vm.$refs.listalarm.style.minHeight = '0px';
  	  	vm.$refs.fee.style.height='0px';
  	  	vm.$refs.fee.style.minHeight='0px';
        var type = e.status;
        var url = '../power/general/generalByType/' + companyId + '/' + type;
        var page = $("#jqGrid").jqGrid('getGridParam', 'page');
        ...
        $("#jqGridAlarm").jqGrid('setGridParam', {page: page}).trigger("reloadGrid");
    	  ...
    	  this.$root.$emit('clear-month-fee');
    	  vm.$refs.list.style.height = '0px';
          vm.$refs.list.style.minHeight = '0px';
          vm.$refs.listalarm.style.height = '0px';
          vm.$refs.listalarm.style.minHeight = '0px';
          vm.$refs.fee.style.height='auto';
    	  vm.$refs.fee.style.minHeight='450px';
      } else {
        vm.$refs.list.style.height = '0px';
        vm.$refs.list.style.minHeight = '0px';
        vm.$refs.listalarm.style.height = '0px';
...
var vm = new Vue({
  ...
  mounted: function() {
    ...
    var flag = sessionStorage.getItem('customer#warning#flag');
    if(flag == 1){
    	this.$root.$emit('clear-date-alarm');
  	  	$("#list").css({"height":"0px"});
  	  	$("#list").css({"minHeight":"0px"});
  	  	$("#listalarm").css({"height":"0px"});
  	  	$("#listalarm").css({"minHeight":"450px"});
  	    $("#fee").css({"height":"0px"});
	  	$("#fee").css({"minHeight":"0px"});
	  	$("#tabs").attr("style","display:none;");
	  	
	  	//$("#tab_warning").attr("selected", "true");
	  	//$("#test2").attr("style","display:none;");
        var page = $("#jqGridAlarm").jqGrid('getGridParam', 'page');
        $("#jqGridAlarm").jqGrid('setGridParam', {page: page}).trigger("reloadGrid");
        sessionStorage.removeItem("customer#warning#flag");
    }
