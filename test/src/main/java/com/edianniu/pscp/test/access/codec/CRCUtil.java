 ...
 * @date 2017年9月11日 下午12:06:26
 * @version V1.0
 */
public class CRCUtil {
	// X16 + X12 + X5 + 1 余式表 31*8 
	static int[] CRCTABLES = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
	 ...
	 * @return
	 */
	public static int getCrc16(byte[] content, int len) {
		int crc = 0;
		int by = 0;
		for (int i = 0; i < len; i++) {
			by = (crc >> 8) & 0xff;
			crc = (crc & 0xffff) << 8;
			crc = (crc ^ CRCTABLES[(content[i] ^ by) & 0xff]) & 0xffff;
		}
	...
	public static short getCrc(byte[] data) {
        ...
        for (int i = 0; i < length; i++) {
            crc = ((crc & 0xFF) << 8) ^ CRCTABLES[(((crc & 0xFF00) >> 8) ^ data[i]) & 0xFF];
        }
        crc = crc & 0xFFFF;
        return (short)crc;
    }
	...
	public static void main(String[] args) {
		...
		int crc = CRCUtil.calcCrc16(new byte[] { 0x02, 0x05, 0x00, 0x03,
				(byte) 0xff, 0x00 });
		System.out.println(crc);
		System.out.println(String.format("0x%04x", 417));
	}
